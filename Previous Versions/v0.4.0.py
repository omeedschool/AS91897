...
class DataTable(ttk.Frame):
    ...
    def create_widgets(self):
        ...
        self.treeview = ttk.Treeview(self, columns=(
            'Name', 'Receipt', 'Item', 'Quantity'), show='headings', yscrollcommand=tree_scroll.set)
    ...
    def item_select(self, event):
        ...
        self.selected_data = [tuple(str(value) for value in event.widget.item(item)[
                                    'values']) for item in self.selected_id]
        ...
        if self.selected_id:
            ...
            if response:
                selected_indexes = [self.treeview.index(
                    item) for item in self.selected_id]
