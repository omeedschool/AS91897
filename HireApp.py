...
class DataTable(ttk.Frame):
    ...
    def __init__(self, parent):
        ...
        self.grid(row=1, column=1, sticky="nsew", padx=5, pady=0)

        self.create_widgets()

        self.update_tree_view(data_list)

    def create_widgets(self):
        self.tree_scrollbar = ttk.Scrollbar(self)
        ...
        self.tree_scrollbar.grid(column=1, row=0, sticky="ns")

        self.tree_scrollbar.config(command=self.tree_view.yview)

        self.tree_view.bind("<<TreeviewSelect>>", self.item_select)
        self.tree_view.bind("<Delete>", self.delete_items)
        self.tree_view.bind("<Button-3>", self.right_click)

    def create_tree_view(self):
        self.tree_view = ttk.Treeview(self, columns=(
            "Name", "Receipt", "Item", "Quantity"), show="headings", yscrollcommand=self.tree_scrollbar.set)

        self.tree_view.heading(
            "Name", text="Name", command=lambda: self.sort_column(self.tree_view, "Name"))
        self.tree_view.column("Name", anchor="center", minwidth=50)

        self.tree_view.heading("Receipt", text="Receipt", command=lambda: self.sort_column(
            self.tree_view, "Receipt"))
        self.tree_view.column("Receipt", anchor="center",
                             minwidth=50, width=150)

        self.tree_view.heading(
            "Item", text="Item", command=lambda: self.sort_column(self.tree_view, "Item"))
        self.tree_view.column("Item", anchor="center", minwidth=50)

        self.tree_view.heading("Quantity", text="Quantity", command=lambda: self.sort_column(
            self.tree_view, "Quantity"))
        self.tree_view.column("Quantity", anchor="center",
    ...
    def item_select(self, event):
        ...
        if self.selected_data:
            self.master.notebook.form.update_entry_box(self.selected_data)
            self.master.navigation.update_display(self.selected_data)

    def update_item(self):

        name = self.master.notebook.form.name_entry.get()
        receipt = self.master.notebook.form.receipt_entry.get()
        item = self.master.notebook.form.clicked.get()
        quantity = self.master.notebook.form.quantity_entry.get()
        ...
        if not name.isalpha():
            messagebox.showwarning(
                "Invalid Name", "Name should not contain symbols or numbers."
            )
            return
    ...
    def update_tree_view(self, tree_view_data):

        self.tree_view.delete(*self.tree_view.get_children())

        for data in tree_view_data:
            self.tree_view.insert("", "end", values=data)
    ...
    def search(self, query):
        ...
        self.update_tree_view(searched_items)
    ...
    def delete_items(self, event=None):
        ...
        self.selected_ids = self.tree_view.selection()

        if self.selected_ids:
            response = messagebox.askyesno(
                "Delete Items", "Are you sure you want to delete the selected items?")
            if response:
                selected_indexes = [self.tree_view.index(
                ...
                self.update_tree_view(data_list)
                self.master.notebook.form.clear_entry_fields()
                self.master.navigation.update_display(None)
        else:
            messagebox.showwarning(
    ...
    def sort_column(self, tree_view, column):
        ...
        tree_view.heading(column, text=f"{column} {sort_arrow}")
...
class ItemTab(ttk.Frame):
    ...
    def create_widgets(self):
        ...
        self.add_button = ttk.Button(
            self, text="Add", command=self.add_item_type)
        self.remove_button = ttk.Button(
            self, text="Remove", command=self.remove_item_type)

        item_tree_scrollbar = ttk.Scrollbar(self)

        self.item_type_tree = ttk.Treeview(self, columns=(
            "Item"), show="headings", yscrollcommand=item_tree_scrollbar.set)

        self.item_type_tree.heading("Item", text="Item Type")
        self.item_type_tree.column("Item", anchor="center")

        item_tree_scrollbar.config(command=self.item_type_tree.yview)

        self.item_name_label.grid(row=0, column=0, pady=5, sticky='n')
        self.item_name_entry.grid(row=0, column=1, pady=5, sticky='n')

        self.add_button.grid(row=1, column=0, sticky="n",
                             padx=10, pady=5, ipadx=25, ipady=2)
        self.remove_button.grid(
            row=1, column=1, sticky="n", padx=10, pady=5, ipadx=25, ipady=2)

        self.item_type_tree.grid(column=0, row=2, sticky="nsew",
                                 columnspan=2, padx=5)
        item_tree_scrollbar.grid(column=2, row=2, sticky="ns")
    ...
    def add_item_type(self):
        ...
        if not item.isalpha():
            messagebox.showwarning(
                "Invalid Item", "Item type name should not contain numbers or symbols")
            return
        else:
            item_type_list.append(item)

        self.update_item_tree(item_type_list)

    def remove_item_type(self, event=None):

        self.selected_ids = self.item_type_tree.selection()

        if self.selected_ids:
            response = messagebox.askyesno(
                "Delete Items", "Are you sure you want to delete the selected items?")
            if response:
                selected_indexes = [self.item_type_tree.index(
                    item) for item in self.selected_ids]
                selected_indexes.sort(reverse=True)
    ...
    def update_item_tree(self, tree_view_data):

        self.item_type_tree.delete(*self.item_type_tree.get_children())

        for data in tree_view_data:
            self.item_type_tree.insert("", "end", values=data)

        self.master.form.update_options(item_type_list)
...
class Form(ttk.Frame):
    ...
    def update_options(self, data):
        ...
        menu = self.item_option_menu['menu']
        menu.delete(0, 'end')
    ...
    def create_widgets(self):
        ...
        self.item_option_menu = ttk.OptionMenu(
            self, self.clicked, *self.options)
        ...
        self.item_option_menu.grid(row=2, column=1, pady=5)
    ...
    def update_multiple_items(self):
        ...
        if response:
            self.master.master.data_table.update_item()
            self.master.master.toolbar.clear_search()
    ...
    def add_data(self):
        ...
        if item == "Select-Item":
            messagebox.showwarning(
                "Invalid Item", "Please select an item"
            )
            return

        for item_data in data_list:
            if (name, int(receipt), item) == (item_data[0], item_data[1], item_data[2]):
                prompt = messagebox.askyesnocancel(
                    "Duplicate Found", "A duplicate item was found, are you sure you want to add this item?")
                if prompt is None:
                    return
                elif prompt:
                    break
        ...
        self.master.master.data_table.update_tree_view(data_list)

    def return_item(self):
        self.master.master.data_table.return_items()
    ...
    def update_entry_box(self, selected_data=None):
        ...
        if selected_data:
            ...
            self.clicked.set(selected_data[0][2])
            self.quantity_entry.insert(0, selected_data[0][3])

    def update_item(self):

        if len(self.master.master.data_table.selected_data) > 1:
    ...
    def clear_entry_fields(self):
        ...
        self.quantity_entry.delete(0, "end")
        self.master.master.data_table.selected_data = None
...
class Notebook(ttk.Notebook):
    def __init__(self, parent):
        ...
        self.item_tab = ItemTab(self)
        self.form = Form(self)

        self.add(self.form, text='Menu')
        self.add(self.item_tab, text='Items')

        self.grid(column=0, row=1, sticky="new", padx=(10, 5), pady=0)
...
class Navigation(ttk.Frame):
    ...
    def navigate_left(self):
        ...
        tree = self.master.data_table.tree_view
...
class Toolbar(ttk.Frame):
    ...
    def __init__(self, parent):
        ...
        self.grid(row=0, column=1, padx=(5, 20), pady=5, sticky="ew")
